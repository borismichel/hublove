...
export class ClaudeCodeEngine implements AIEngine {
  async convert(opts: {
    ...
    onProgress("convert", "Running Claude Code (this may take a few minutes)...");
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.on("error", (err) => reject(new Error(`Claude Code failed: ${err.message}`)));
      child.on("close", (code) => {
...
Create all files directly in the theme directory.`;
  ...
  private scanGeneratedFiles(themePath: string): GeneratedAssets {
    ...
    // Find shared CSS (any new *-theme.css or *-page.css in css/)
    ...
    // Find shared JS (any new *-animations.js in js/)
    ...
    // Find new template (any lp-*.html or *-landing*.html in templates/)
